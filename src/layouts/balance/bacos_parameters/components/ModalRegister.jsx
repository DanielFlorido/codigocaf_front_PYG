...
const ModalRegister = ({
  ...
  const {
    ...
    error: errorEditData,
    putData,
  } = usePut(`${ENDPOINTS.BANK_PARAMETERS_ID}${formData?.id}`, {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      codigoACH: formData?.codigoACH || "",
    },
    validationSchema: Yup.object({
      nombre: Yup.string().required("Requerido"),
      codigoACH: Yup.string().required("Requerido"),
    }),
    ...
    onSubmit: async (values) => {
      if (isEditing) {
        ...
        try {
          ...
          const response = await postData(ENDPOINTS.BANK_PARAMETERS_ID, payload, {
            "Content-Type": "application/json",
          });
          ...
          {isDeleting ? (
            ...
            <MDBox p={1}>
              <Grid container spacing={2}>
                ...
                <Grid item xs={12}>
                  <MDTypography variant="p">Código ACH:</MDTypography>
                  <TextField
                    fullWidth
                    name="codigoACH"
                    value={formik.values.codigoACH}
                    onChange={formik.handleChange}
                    error={formik.touched.codigoACH && Boolean(formik.errors.codigoACH)}
                    helperText={formik.touched.codigoACH && formik.errors.codigoACH}
